...
<script>
  ...
  export let backgroundTransitionInterval = 2; // Change background every N text lines
  ...
  // Helper function to get background image index based on text line index
  function getBackgroundImageIndex(textLineIndex) {
    return Math.floor(textLineIndex / backgroundTransitionInterval);
  }
  ...
  function initAnimations() {
    ...
    // Set initial states for background images
    ...
    scrollWrappers.forEach((wrapper, index) => {
      ...
      const textElement = wrapper.querySelector('.text-h2');
      const backgroundIndex = getBackgroundImageIndex(index);
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          if (index % backgroundTransitionInterval === 0 && backgroundImages[backgroundIndex]) {
            const currentBgImage = backgroundImages[backgroundIndex];
            const prevBgImage = backgroundImages[backgroundIndex - 1];
            
            // Fade in current background image
            gsap.killTweensOf(currentBgImage);
            gsap.to(currentBgImage, {
              opacity: 1,
              scale: 1,
              duration: 1.2,
              ease: "power2.out",
        ...
        onEnterBack: () => {
          ...
          // Handle background image transitions on scroll back
          if (index % backgroundTransitionInterval === 0 && backgroundImages[backgroundIndex]) {
            const currentBgImage = backgroundImages[backgroundIndex];
            const nextBgImage = backgroundImages[backgroundIndex + 1];
            ...
            // Fade out next background image
            if (nextBgImage) {
              gsap.killTweensOf(nextBgImage);
              gsap.to(nextBgImage, {
                opacity: 0,
                scale: 1.1,
                duration: 1.2,
    ...
    // Parallax effects for foreground images
...
<section id="base" class="section base">
  ...
  <div class="background-images-container">
    {#each Array.from({length: Math.ceil(lines.length / backgroundTransitionInterval)}) as _, bgIndex}
      <div 
        class="background-image" 
        style="background-image: url('{getImageUrl(bgIndex)}');"
      >
