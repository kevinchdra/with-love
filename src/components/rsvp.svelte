<script>
  ...
  async function handleRSVP(formData) {
    if (isDemo) {
      // Show demo success message instead of submitting to database
      return {
        success: true,
        message: "This is a demo - RSVP submission simulated!"
      };
    }};
  ...
  async function fetchWishes() {
    ...
    if (fetchError) {
      ...
      wishesWall = data.filter(w => w.wishes && w.wishes.trim() !== '');
    }
...
async function handleFinalSubmit() {
  ...
  try {
    await handleSubmit();   // Save to Supabase
...
<div class="flex flex-col items-center px-8 pt-10 sm:pt-14 md:pt-16 text-white snap-start">
  <div class="max-w-xl w-full space-y-6 fade-in">
    ...
    <hr class="mt-[-6px] sm:mt-[-8px] md:mt-[-10px]">
      <div class="">
        <p class="text-lg mb-10 sm:mb-8 md:mb-12 mb-12sm:text-xl md:text-2xl font-['Sangbleu_Light']">We invite you to confirm your presence at our celebration and to share your heartfelt wishes with us. Please RSVP by Sunday, 28 July 2026, to help us prepare for your arrival.</p>
      </div>
